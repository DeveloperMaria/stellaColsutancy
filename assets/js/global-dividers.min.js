jQuery(window).on("elementor/frontend/init", () => {
  var e = (e) => {
    var d = e.attr("data-pix-dividers");
    void 0 !== d &&
      !1 !== d &&
      (e.hasClass("elementor-column")
        ? (e
            .find("> .elementor-column-wrap")
            .addClass("pix-elementor-section-dividers"),
          e
            .find("> .elementor-column-wrap")
            .prepend(e.attr("data-pix-dividers")))
        : (e.addClass("pix-elementor-section-dividers"),
          e.prepend(e.attr("data-pix-dividers"))),
      e.attr("data-pix-dividers", ""),
      pix_animation(e, !0),
      e.find(".pix-shape-dividers").each(function () {
        if (!jQuery(this).hasClass("loaded")) {
          let e = new dividerShapes(this);
          e.initPoints(), jQuery(this).addClass("loaded");
        }
      }));
  };
  elementorFrontend.hooks.addAction("frontend/element_ready/section", e),
    elementorFrontend.hooks.addAction("frontend/element_ready/column", e),
    elementorFrontend.hooks.addAction("frontend/element_ready/container", e);
});
//# sourceMappingURL=global-dividers.min.js.map
